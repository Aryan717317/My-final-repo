...
export default function SkillsSection() {
  ...
  return (
    <section id="skills" className="relative min-h-screen py-24 px-8 md:px-12 w-full max-w-[1600px] mx-auto z-20 flex flex-col justify-center bg-[#09090B]">
      
      {/* Glow effect */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-[#6366F1]/5 rounded-full blur-[150px] pointer-events-none"></div>
      ...
      <div className="flex flex-col items-start mb-16 relative z-10">
        <motion.div
          ...
          className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/[0.03] border border-white/[0.08] text-xs font-mono text-[#8B5CF6] mb-4"
        >
        ...
        >
          My Skillset<span className="text-[#8B5CF6]">.</span>
        </motion.h2>
        <motion.p
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
      ...
      {/* Tabs list */}
      <div className="flex flex-wrap gap-2 p-1.5 rounded-2xl bg-white/[0.02] border border-white/[0.06] mb-12 w-fit relative z-10">
            ...
            >
              {isActive && (
                <motion.div
                  ...
                  className="absolute inset-0 bg-[#111217] border border-white/[0.08] rounded-xl -z-10"
                  transition={{ type: "spring", stiffness: 380, damping: 30 }}
                />
              )}
              <span className={isActive ? "text-[#8B5CF6]" : "text-slate-400"}>
                {category.icon}
              </span>
              {category.name}
            </button>
              ...
              >
                {category.skills.map((skill, index) => (
                  <motion.div
                    ...
                    className="p-6 rounded-2xl bg-[#111217]/60 backdrop-blur-md border border-white/[0.08] hover:bg-[#111217] transition-all group duration-300 flex flex-col justify-between"
                  >
                    <div>
                      <div className="flex items-start justify-between mb-4">
                        <h4 className="text-xl font-bold text-white tracking-tight">{skill.name}</h4>
                        <span className="text-[11px] font-mono font-bold tracking-wider px-2.5 py-1 rounded-full bg-[#8B5CF6]/10 text-[#A855F7] border border-[#8B5CF6]/20">
                          {skill.level}
                        </span>
                      </div>
                    ...
                    <div className="mt-6 pt-4 border-t border-white/[0.04] flex justify-end">
                      <span className="text-xs font-mono text-white/20 group-hover:text-[#8B5CF6]/50 transition-colors">
                        // verified mastery
                      </span>
                    </div>
                  </motion.div>
